...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import zrl_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "zrl_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"zrl", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "zrl_jll", "src", "zrl_jll.jl"))
        @info("zrl_ll dev'ed out to /depot/dev/zrl_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("9fe51eb0-a099-5680-98c2-fe494cf0c162"))
platforms = [Pkg.Artifacts.unpack_platform(e, "zrl", artifacts_toml) for e in artifacts["zrl"]]
...
if best_platform === nothing
    @debug("Unable to load zrl; unsupported platform $(triplet(platform_key_abi()))")
else
